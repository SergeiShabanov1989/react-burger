describe('Application', function () {
  ...
  it('should close modal after click close button', () => {
    cy.get('[data-testid="bun"]').first().click();
    cy.get('[data-testid="ingredient-name"]').as('ingredient').should('have.text', 'Ингредиент 1');
  ...
  it('should drop on drop-target', () => {
    ...
    cy.get('[data-testid="drop-target"]').as('dropTarget').trigger('drop');
    cy.get('@dropTarget').should('have.text', 'Ингредиент 1 (вверх)1255');
  });
